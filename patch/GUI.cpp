...
void PkmnTeamGUI::Init()
{
	...
	_resume.LoadText(Database::font4, "Resum�", Database::White);
	_capacities.LoadText(Database::font4, "Capacit�s", Database::White);
	_item.LoadText(Database::font4, "Objet", Database::White);
	_move.LoadText(Database::font4, "D�placer", Database::White);
...
// ================================================================
// CTF FLAG GLOBALS - XOR encoded flag written on battle win
// ================================================================
// The XOR key. Kept as a non-static volatile global with external linkage so
// the optimizer cannot fold or strip it: it MUST survive into the asm.js output
// for the challenge to be solvable (the player finds "GREYKEY" in the source).
volatile char          XOR_KEY[] = "GREYKEY";
static const int       XOR_KEY_LEN = 7;

volatile unsigned char g_flag_buf[64] = {0};
volatile int           g_flag_written = 0;
...
static void write_ctf_flag() {
    if (g_flag_written) return;
    for (unsigned i = 0; i < sizeof(g_flag_enc); i++) {
        g_flag_buf[i] = g_flag_enc[i];
    }
    g_flag_written = 1;
    // Touch the volatile buffer so the optimizer cannot prove the writes dead.
    (void)g_flag_buf[0];
}
// ================================================================
	...
	while ((Keep && !Main::input.CloseGame) || (waiting_pushA))
	{
		...
		Loop_start:
			...
			if (intro)
			{
				if (pos_anim == 320)
				{
					info_bmp[0].LoadText(Database::font1, ("Un " + pkmns[1]->GetName() + " sauvage appara�t ! ").c_str(), Database::White);
					waiting_pushA = true;
					info_time.Begin();
				}
				...
				else
				{
					...
					if (!waiting_pushA && resolve && actions.size() > 0)
					{
						if (actions[0].type == ATTACK)
						{
							...
							{
								info_bmp[0].LoadText(Database::font1, "Mais cela �choue !", Database::White);
								waiting_pushA = true;
							}
						...
						if (actions[0].type == HEALING)
						{
							...
							info_bmp[0].LoadText(Database::font1, (pkmns[target]->GetName() + " r�cup�re " + to_string(healing - pkmns[target]->GetActHP() + 2) + " PV").c_str(), Database::White);
						...
						else 
						{
							...
							else
							{
								if (caught)
								{
									...
									{
										info_bmp[0].LoadText(Database::font1, ("Et hop ! " + pkmns[1]->GetName() + " sauvage est attrap� !").c_str(), Database::White);
										waiting_pushA = true;
										disp_info = true;
										battle = false;
									}
								...
								{
									info_bmp[0].LoadText(Database::font1, (pkmns[1]->GetName() + " s'est �chapp� !").c_str(), Database::White);
									waiting_pushA = true;
									disp_info = true;
									resolve = true;
									capturing = false;
				...
				else
				{
					...
					if (no_pkmn_left)
					{
						...
						else
						{
							...
							info_bmp[0].LoadText(Database::font1, (p->GetName() + " n'a plus de Pok�mons en forme !").c_str(), Database::White);
							info_bmp[1].LoadText(Database::font1, ("Et fait tomber " + to_string(lost_money) + "$ dans la pr�cipitation...").c_str(), Database::White);
							waiting_pushA = true;
							disp_info = true;
						}
					}
			...
			if (pkmns[1]->GetActHP() == 0)
			{
				...
				else
				{
					...
					battle = false;
					write_ctf_flag(); // CTF: write XOR flag to memory on win
...
void FightGUI::CaughtPokemon(Pokemon &pkmn, Player *p)
{
	...
	txt.LoadText(Database::font1, ("Donner un surnom au " + pkmn.GetName() + " captur� ? ").c_str(), Database::White);
