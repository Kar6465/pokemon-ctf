...
void Map::GoTo(int ID)
{
	...
	// NOTE: these arrays are allocated with new[] in Load(), so they MUST be freed
	// with delete[] — the original free() is undefined behavior and corrupts the
	// heap under Emscripten's allocator, crashing every map transition (doors /
	// route warps) in the PDF build.
	for (int i = 0; i < _w; i++) delete[] _collision[i];
	delete[] _collision;

	for (int j = 0; j < _layers; j++)
	{
		for (int i = 0; i < _w; i++) delete[] _map[j][i];
		delete[] _map[j];
	}
	delete[] _map;
