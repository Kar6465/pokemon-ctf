...
// In a PDF/browser there are no SDL key events. JS (pokemon_pre.js) calls
// pokemon_key(code, down) to drive this table. Codes are ASCII for letters and
// digits; arrows and modifiers use the KC_* codes below. Inputs::Update-
// KeyboardInputs reads this table instead of SDL_PollEvent.
static volatile unsigned char g_key_down[256] = { 0 };
enum { KC_SHIFT = 16, KC_ENTER = 13, KC_ESC = 27, KC_SPACE = 32,
       KC_UP = 128, KC_DOWN = 129, KC_LEFT = 130, KC_RIGHT = 131 };
extern "C" EMSCRIPTEN_KEEPALIVE void pokemon_key(int code, int down)
{
	if (code >= 0 && code < 256) g_key_down[code] = down ? 1 : 0;
}
#endif
...
#if !defined(_WIN32)
// ---- Portable no-op gamepad (no XInput off Windows). Keyboard play is used. ----
...
void Gamepad::Vibrate(float, float, double) {}
...
#ifdef __EMSCRIPTEN__
	// Mirror of the SDL mapping below, but sourced from the JS key table.
	_Keys[Up][Actual]      = g_key_down[KC_UP];
	_Keys[Down][Actual]    = g_key_down[KC_DOWN];
	_Keys[Left][Actual]    = g_key_down[KC_LEFT];
	_Keys[Right][Actual]   = g_key_down[KC_RIGHT];
	_Keys[A][Actual]       = g_key_down['a'];
	_Keys[Q][Actual]       = g_key_down['q'];
	_Keys[Z][Actual]       = g_key_down['z'];
	_Keys[E][Actual]       = g_key_down['e'];
	_Keys[D][Actual]       = g_key_down['d'];
	_Keys[R][Actual]       = g_key_down['r'];
	_Keys[F][Actual]       = g_key_down['f'];
	_Keys[P][Actual]       = g_key_down['p'];
	_Keys[S][Actual]       = g_key_down['s'];
	_Keys[W][Actual]       = g_key_down['w'];
	_Keys[L][Actual]       = g_key_down['l'];
	_Keys[T][Actual]       = g_key_down['t'];
	_Keys[ONE][Actual]     = g_key_down['1'];
	_Keys[TWO][Actual]     = g_key_down['2'];
	_Keys[THREE][Actual]   = g_key_down['3'];
	_Keys[FOUR][Actual]    = g_key_down['4'];
	_Keys[FIVE][Actual]    = g_key_down['5'];
	_Keys[SIX][Actual]     = g_key_down['6'];
	_Keys[SEVEN][Actual]   = g_key_down['7'];
	_Keys[EIGHT][Actual]   = g_key_down['8'];
	_Keys[NINE][Actual]    = g_key_down['9'];
	_Keys[ZERO][Actual]    = g_key_down['0'];
	_Keys[ESCAPE][Actual]  = g_key_down[KC_ESC];
	_Keys[Space][Actual]   = g_key_down[KC_SPACE];
	_Keys[L_SHIFT][Actual] = g_key_down[KC_SHIFT];
	_Keys[Return][Actual]  = g_key_down[KC_ENTER];
#else
